# src/main_modal.py
# Main workflow using Modal cloud training for the GRU component
# Demonstrates production-ready cloud-accelerated training
# RELEVANT FILES: main.py, modal_gru.py, hybrid.py
...
def main():
    print("=" * 50)
    print("GARCH-GRU Simulation Pipeline (Modal Cloud)")
    print("=" * 50)
    ...
    # Step 4: Fit Hybrid Model with Modal Cloud Training
    print("\n4. Training GARCH-GRU hybrid model on Modal cloud...")
    print("   This will use GPU acceleration and automatically shutdown when complete...")
    hybrid_model = GARCHGRUHybrid(use_modal=True)
    hybrid_model.fit(spy_data, log_returns, gkyz_volatility)
    print(f"   Training complete! Time: {hybrid_model.get_training_time():.2f} seconds")
    ...
    plt.savefig('simulated_paths_modal.png', dpi=300, bbox_inches='tight')
    
    plot_volatility_comparison(
        garch_model.conditional_volatility(),
        gkyz_volatility
    )
    plt.savefig('volatility_comparison_modal.png', dpi=300, bbox_inches='tight')
    
    plot_return_distributions(garch_paths, hybrid_paths)
    plt.savefig('return_distributions_modal.png', dpi=300, bbox_inches='tight')
    
    plot_model_comparison_table(comparison)
    plt.savefig('model_comparison_modal.png', dpi=300, bbox_inches='tight')
    
    print("   Plots saved with '_modal' suffix!")
    
    # Show plots
    plt.show()
    
    print("\n" + "=" * 50)
    print("Modal Cloud Simulation Complete!")
    print(f"GRU Training Time: {hybrid_model.get_training_time():.2f} seconds")
    print("=" * 50)
    ...
    return {
        ...
        'training_time': hybrid_model.get_training_time()
    }
